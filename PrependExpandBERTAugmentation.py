...
# Define paths
txt_file = '/content/drive/MyDrive/Thesis/DataAugmentation/data/processedData/data_15_train.txt'
augmented_data_path = '/content/drive/MyDrive/Thesis/DataAugmentation/augmentedData/2015'
...
def fill_masked_words(mask_indices, words, model, tokenizer, top_k=5):
    ...
    for mask_token_index, original_word in zip(mask_token_indices, original_words):
        ...
        for token_id in top_k_ids:
            candidate_word = tokenizer.decode([token_id], skip_special_tokens=True).strip()
            if candidate_word != original_word:
                replacement_word = candidate_word
                break
        if not replacement_word:
            replacement_word = tokenizer.decode([top_k_ids[0]], skip_special_tokens=True).strip()
...
def prepare_and_save_augmented_data(method_type, augmentation_method, postag=None):
    if method_type == 'PREPEND':
      model_path = './models/fine_tuned_PREPENDBERT_2015_model'
      tokenizer_path = './models/fine_tuned_PREPENDBERT_2015_tokenizer'
    elif method_type == 'EXPAND':
      model_path = './models/fine_tuned_EXPANDBERT_2015_model'
      tokenizer_path = './models/fine_tuned_EXPANDBERT_2015_tokenizer'
    ...
    if augmentation_method == 'POS':
        augmented_txt_path = os.path.join(augmented_data_path, f'{method_type}BERT2015augmented_data_{augmentation_method}_{postag}.txt')
    else:
        augmented_txt_path = os.path.join(augmented_data_path, f'{method_type}BERT2015augmented_data_{augmentation_method}.txt')
